...
use kernway_security::session::{self, MemorySessionStore, SessionConfig, SessionManager};
...
pub fn build_app(addr: &str) -> KernwayApp {
    ...
    engine
        .add("protected", PROTECTED_PAGE)
        .expect("protected template");
    ...
    KernwayApp::builder()
        ...
        .get("/protected", move |_req: Request, scope: &RequestScope| {
            let ctx = scope
                .get::<SecurityContext>()
                .expect("auth middleware set a SecurityContext");
...
fn render_login(engine: &Kernleaf) -> Response {
    ...
    let html = engine
        .render_with(
            "login",
            &Value::Null,
            &RenderContext::new().csrf(token.as_str()),
        )
        .unwrap_or_else(|e| format!("<p>template error: {e}</p>"));
...
async fn do_login(req: &Request, sessions: &SessionManager) -> Response {
    ...
    if username != "alice" || password != "secret" {
        return HtmxResponse::new("<p id=\"result\">Wrong username or password.</p>")
            .into_response();
    }

    let token = match sessions
        .login("alice", vec!["ADMIN".to_string()], "web")
        .await
    {
        Ok(t) => t,
    ...
    resp.headers
        .insert("set-cookie", &session::set_cookie(&token, false));
    resp
}
