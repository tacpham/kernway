...
fn bans_survive_a_restart() {
    Executor::new()
        ...
        .block_on(async {
            ...
            let seed = PersistentBans::restore(RedisBanStore::new(addr))
                .await
                .unwrap();
            ...
            let a = PersistentBans::restore(RedisBanStore::new(addr))
                .await
                .unwrap();
            ...
            assert!(
                bans.is_banned(None, Some("Some EVILBOT/2.0")),
                "UA-contains ban restored (case-insensitive)"
            );
            assert!(
                !bans.is_banned(Some(ip("8.8.8.8")), Some("Mozilla/5.0")),
                "a clean request is not banned"
            );

            // An unban also persists across a restart.
            b.unban_ip(ip("203.0.113.7")).await.unwrap();
            drop(b);
            let c = PersistentBans::restore(RedisBanStore::new(addr))
                .await
                .unwrap();
            assert!(
                !c.bans().is_banned(Some(ip("203.0.113.7")), None),
                "unban restored"
            );
            assert!(
                c.bans().is_banned(Some(ip("198.51.100.42")), None),
                "other bans still there"
            );
