...
fn precompressed_variant_is_negotiated_over_a_socket() {
    ...
    assert!(
        gz.contains("content-encoding: gzip\r\n"),
        "missing CE: {gz:?}"
    );
    assert!(
        gz.contains("vary: Accept-Encoding\r\n"),
        "missing Vary: {gz:?}"
    );
    assert!(
        gz.contains("content-type: text/css; charset=utf-8\r\n"),
        "type: {gz:?}"
    );
    assert!(
        gz.ends_with("SENTINEL-GZIP-BYTES"),
        "should serve the .gz bytes: {gz:?}"
    );
    ...
    assert!(
        identity.contains("vary: Accept-Encoding\r\n"),
        "missing Vary: {identity:?}"
    );
    assert!(
        !identity.contains("content-encoding:"),
        "identity must not claim an encoding: {identity:?}"
    );
    assert!(
        identity.ends_with("body { color: rebeccapurple; }"),
