...
pub(crate) fn decompress(resp: &mut Response) -> Result<(), HttpError> {
    let Some(encoding) = resp.header("content-encoding").and_then(Encoding::from_token) else {
    ...
    resp.body = kernway_compress::decode(&resp.body, encoding).map_err(|e| HttpError::Protocol(e.0))?;
