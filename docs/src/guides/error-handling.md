...
Each layer has its own error type. Use the `thiserror::Error` derive macro.
...
`#[from]` generates the `From` trait automatically → use `?` without needing `.map_err()`:
...
`#[validated]` + `Validated<Json<T>>` automatically returns RFC 7807 when validation fails — no separate handler needed:
...
Custom log level by error type:
