...
Same result: each thread accepts connections and processes them independently. Overhead < 2% (this is exactly how Kestrel — the ASP.NET Core server — works).
...
**Issue**: CPU pinning APIs differ across platforms.

| Platform | API | Result |
|---|---|---|
| Linux | `sched_setaffinity()` | Hard pin — the kernel does not migrate the thread |
| macOS | `pthread_mach_thread_np` + thread_policy | Hint only — the macOS scheduler can override it |
...
**Solution**: Unified graceful shutdown API.
...
**Initial concern**: We thought mio on Windows used a wepoll emulation layer.
...
Each crate with platform-specific I/O code must follow this pattern:
...
Each platform: unit tests + integration test echo server + HTTP request/response roundtrip.
