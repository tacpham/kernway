...
> **Rule zero**: Before implementing any feature, read the corresponding RFC/spec first.

## Mandatory principles

1. **Read the spec first** — each RFC section = one test case
2. **MUST/REQUIRED in the RFC** — no exceptions, no "skip for now"
3. **SHOULD in the RFC** — clearly document any non-implementation and why
...
| A07 Auth Failures | Default rate limiting, constant-time token comparison |
| A08 Data Integrity | Request signature, CSRF token |

**XSS Prevention (kernleaf):**
- All `kw:text` values are HTML-escaped by default
- Raw output is only available through explicit `kw:utext` (unescaped text — intentionally explicit naming)
