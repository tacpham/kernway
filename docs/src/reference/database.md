...
Bridges blocking DB operations (diesel + r2d2) into the async executor via `spawn_blocking`.
...
- The `spawn_blocking` pattern adds one channel send/recv per query (~microseconds) — acceptable overhead
- If the community wants native async support: implement the `DbPool` trait with a custom driver — no need to fork Kernway
