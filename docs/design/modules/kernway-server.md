...
A pre-compiled binary loads the user app as a dynamic library (`.so`/`.dll`/`.dylib`).  
This enables hot reload: rebuild the app → the server reloads without restarting.
...
The user app exposes its entry points through a stable ABI (no name mangling, no Rust ABI instability):
...
| HTTP sessions | Lost if in-memory; preserved when using Redis |
| Environment variables | Preserved |

**Best practice for development**: use DB seeding to reset state after reload.
...
- Windows: `.dll` loading/unloading has fewer limitations than Linux — tested, but less stable
- Debug symbols do not reload — attaching a debugger requires a restart
- A panic in app code means kernway-server catches and logs it instead of crashing the server

## Production

Hot reload is only for `kernway dev`. In production (`kernway build`), a single static binary is produced with no dynamic loading.
